...
| **IN-05b** | Family Income | What is your family's total annual income from all sources? | `gr.Dropdown` | Dynamic ranges based on family size (IN-05a). Map to 2023 family-income categories relative to the poverty line. | `POVCAT23` |
...
| **FR-04** | **Cost Drivers** | Compute SHAP values for the plan-around estimate to explain which input features moved the model estimate higher or lower. Display the five largest contributions as inflation-adjusted dollar impacts. |
...
| **UI-02** | **Cost Drivers** | The five answers with the largest absolute SHAP contributions to the plan-around estimate, shown in current dollars. | `gr.Markdown` | **These five answers contributed most to your plan-around estimate:** "Age: 68, +$480; Diabetes: Yes, +$370; Physical health: Good, -$90" |
...
*   **Default Alternative**: When a future MEPS full-year file becomes available, run the deployed model on the newer survey year and repeat the same weighted metrics and subgroup audit. Treat retraining or recalibration as a separate model-refresh decision if caveats persist or worsen.
