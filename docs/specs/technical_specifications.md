...
`warning_flags` must be generated by the prediction service after quantile post-processing and before UI rendering. Flags are product guidance signals, not model-training labels. Threshold-based flags should use fixed thresholds stored with the active model version and derived from the validation prediction distribution, not the locked test set. Compare predictions and thresholds on the same dollar scale; if predictions are inflation-adjusted before flag generation, inflate the stored validation thresholds by the same multiplier.

| Flag | Trigger Logic | Intended UI Use |
| :--- | :--- | :--- |
| `HIGH_PREDICTED_UNCERTAINTY` | Predicted safety cushion (`q90`) is at or above the validation 90th percentile of predicted `q90`, or maps to the highest predicted safety-cushion tier | Tell users to treat the estimate as a rough planning range and emphasize the safety cushion |
...
| `MISSING_OPTIONAL_INPUTS` | One or more optional fields are omitted and imputed | Explain that typical training values were used for skipped inputs |
...
Do not generate user-facing warning flags solely because a user belongs to a subgroup flagged in the offline fairness audit unless the warning is actionable and known at prediction time. Low income, poor mental health, doctorate degree, and near-poor subgroup caveats should remain in documentation, monitoring, and future validation rather than direct per-user warning banners.

Recommended model-version metadata, with placeholder values replaced by model-version validation thresholds:

```json
{
  "warning_thresholds": {
    "predicted_q90_p90": 0.0,
...
9.  **Warning Flags:** Generate structured `warning_flags` from post-processed predictions, validation-derived model thresholds on the same dollar scale, imputed-field metadata, and known-at-prediction inputs such as insurance status.
