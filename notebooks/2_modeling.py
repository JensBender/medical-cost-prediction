...
#         <li><b>Model Monitoring:</b> For the privacy-first MVP product release, do not claim post-launch calibration from app users. True calibration requires actual annual out-of-pocket spending, but collecting linked follow-up outcomes would be unreliable for many users and would weaken the anonymous, zero-retention product requirement.</li>
#         <li><b>MVP Product Monitoring Scope:</b> Track aggregate app health, completion rate, input drift, and prediction drift without storing user-level input rows or prediction records. For prediction drift, monitor the aggregate distribution of predicted q50, q25-q75 range width, q90 safety cushion, high-uncertainty flags, and missingness rates; broad input slices such as insurance status, poverty category, mental health, and chronic-condition count can help explain distribution shifts.</li>
#         <li><b>Future Outcome Monitoring:</b> If the product strategy ever changes to collect actual spend, treat it as a separate opt-in outcome study with explicit consent, data minimization, retention limits, and a privacy review. Because MEPS derives <code>TOTSLF23</code> through detailed expenditure questions, simple user-reported annual spend should be treated as directional unless the collection process can approximate MEPS-quality measurement. Otherwise, use future MEPS releases to test the deployed model on a newer survey year; retraining or recalibration would be a separate model-refresh decision.</li>
#     </ul>
# </div>
#
