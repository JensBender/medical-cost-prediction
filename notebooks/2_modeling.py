...
#     <ul>
#         <li><strong>Actual cost range:</strong> MdAE increases exponentially across actual cost ranges for all three models. At the extremes, the models converge: Zero Costs MdAE ranges from \$29 (XGBoost) to \$90 (Elastic Net), and Super Spenders from \$47,022 (XGBoost) to \$47,843 (Elastic Net). This tight convergence at the extremes confirms that extreme-cost prediction difficulty is a <em>data-driven</em> limitation, not model-specific. Elastic Net shows notably higher errors for Zero Costs (\$90 vs. \$29–\$31 for tree-based models), because its linear structure cannot model the sharp boundary at zero spending.</li>
#         <li><strong>Predicted cost range:</strong> All three models compress their output range, but the degree differs. For predicted "Super Spenders", Elastic Net retains the most signal (\$2,268 MdAE) while XGBoost collapses to \$396 and Random Forest to \$1,281. This reveals that tree-based models' ensemble averaging attenuates tail predictions more aggressively than regularized linear models. The compression is a conservative bias acceptable for an advisory tool but important to disclose.</li>
