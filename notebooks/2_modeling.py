...
#     <br><br>
#     <strong>SHAP Baseline</strong><br>
#     User-facing SHAP explanations compare a user's plan-around estimate with a U.S. adult population reference from MEPS. The comparison baseline is the weighted average q50 prediction across all training rows, measured in 2023 dollars. This baseline is stored with the SHAP metadata so the app can display the same reference value consistently. 
#     <br><br>
#     <strong>Background Data</strong><br>
#     The SHAP background data is a smaller weighted sample of 200-500 rows from the preprocessed training data. It gives SHAP the reference population needed to calculate individual feature contributions without slowing down every prediction request. If the national baseline proves too broad for users, revisit this choice and consider an age-group-specific or otherwise cohort-specific baseline.
#     <br><br>
#     <strong>App/API Implementation Plan</strong>
#     <ol>
#         <li><strong>Create Artifacts:</strong> Save the SHAP background data as <code>app/data/shap_background.parquet</code>and the SHAP baseline and as <code>app/data/shap_metadata.json</code>.</li>
#         <li><strong>During Application Startup:</strong> Load the quantile model, SHAP background sample, and SHAP metadata. Build the SHAP explainer once and keep the baseline in memory for display.</li>
#         <li><strong>At Inference Time:</strong> Preprocess the user row, predict q25/q50/q75/q90, compute SHAP values for q50 only, group encoded columns back to user-facing factors, apply the medical-cost inflation factor to all displayed dollar amounts, and return the top cost drivers.</li>
#     </ol>
# </div>
#
#
...
#     For the quantile model, user-facing SHAP explanations will focus on the q50 plan-around estimate. The q25, q75, and q90 outputs describe uncertainty and planning range, but they should not be mixed into the q50 explanation. 
#     <br><br>
#     <strong>Reference Population and Baseline</strong><br>
#     User-facing SHAP explanations compare a user's plan-around estimate with a U.S. adult population reference from MEPS. The comparison baseline is the weighted average q50 prediction across all training rows, measured in 2023 dollars. This baseline is stored with the SHAP metadata so the app can display the same reference value consistently. The SHAP background data is a smaller weighted sample of preprocessed training rows; it gives SHAP the reference population needed to calculate individual feature contributions without slowing down every app request. If the national baseline proves too broad for users, revisit this choice and consider an age-group-specific or otherwise cohort-specific baseline.
#     <br><br>
#     <strong>App/API Implementation Plan</strong>
#     <ol>
#         <li><strong>Create Artifacts:</strong> Save the SHAP background data and the SHAP population baseline. The background data is a weighted sample of 200-500 rows from the preprocessed training data, persisted as <code>app/data/shap_background.parquet</code>. The SHAP baseline is the weighted average q50 prediction across all training rows, stored as <code>app/data/shap_metadata.json</code> in 2023 dollars with a short description of the reference population used.</li>
#         <li><strong>During Application Startup:</strong> Load the quantile model, SHAP background sample, and SHAP metadata. Build the SHAP explainer once and keep the baseline in memory for display.</li>
#         <li><strong>At Inference Time:</strong> Preprocess the user row, predict q25/q50/q75/q90, compute SHAP values for q50 only, group encoded columns back to user-facing factors, apply the medical-cost inflation factor to all displayed dollar amounts, and return the top cost drivers.</li>
#     </ol>
# </div>
...
# #
# # Per request:
# # shap_values = explainer(user_row_preprocessed)# Planned artifact-building scaffold. Keep this non-executing until the app
# explainability module is implemented and latency-tested.
#
# SHAP_BACKGROUND_N = 300
#
