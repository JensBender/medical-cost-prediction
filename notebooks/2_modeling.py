...
#     </ul>
#     <p style="margin-top:12px"><strong>Caveats:</strong></p>
...
#                 <td style="padding:8px; border:1px solid #d6e8d6;">Use an always-on user warning for rare high costs. Use additional high uncertainty warning message for high cost groups. Since actual costs are unknown at prediction time, use predicted costs to trigger warning message (top 20% safety cushion based on validation data).</td>
#             </tr>
#             <tr>
...
#                 <td style="padding:8px; border:1px solid #d6e8d6;">Document as a monitoring caveat; avoid mental-health-specific user warnings unless a future validation cycle shows a stronger safety-cushion issue.</td>
#             </tr>
#             <tr>
...
#                 <td style="padding:8px; border:1px solid #d6e8d6;">Track in aggregate monitoring and future MEPS validation; do not show income-specific warnings.</td>
#             </tr>
#             <tr>
...
#                 <td style="padding:8px; border:1px solid #d6e8d6;">Recheck in future validation; do not add a degree-specific user warning.</td>
#             </tr>
#             <tr>
#                 <td style="padding:8px; border:1px solid #d6e8d6;">Physical health, race/ethnicity, region</td>
#                 <td style="padding:8px; border:1px solid #d6e8d6;">Wide low-cost intervals for good physical health, Asian respondents, and the West region.</td>
#                 <td style="padding:8px; border:1px solid #d6e8d6;">Use a generic <code>WIDE_PREDICTION_INTERVAL</code> note only when the individual interval is wide: the range is broad because similar profiles varied, so treat it as a planning band and use the safety cushion for a higher-cost year.</td>
#             </tr>
#         </tbody>
#     </table>
# </div>
...
#     <ul style="margin-top:8px">
#         <li><b>Launch Recommendation:</b> Use XGBoost quantile regression as the final model artifact behind the MVP product release for out-of-pocket medical costs. Frame the product as a budgeting aid, not a precise billing or medical advice tool.</li>
...
#         <li><b>Usefulness vs Simple Baseline:</b> The trained model improves on naive population baselines (no ML) for all user-facing outputs. Plan-around skill = 9.8%, typical-range interval skill = 11.2%, and safety-cushion skill = 15.6%. This demonstrates added value of using feature-based model predictions compared with giving every user the same population median, generic q25-q75 range, or population q90 estimate.</li>
#         <li><b>Calibration Decision:</b> Do not add conformalized quantile regression. Test-set calibration passes the predefined gates, so any calibration changes should be evaluated only in a new validation cycle. Use future MEPS releases to test the deployed 2023-trained model on a newer survey year when one becomes available.</li>
...
#                 <li><b>Product Monitoring Scope:</b> Track aggregate app health, completion rate, input drift, and prediction drift without storing user-level input rows or prediction records. For prediction drift, monitor the aggregate distribution of predicted q50, q25-q75 range width, q90 safety cushion, high-uncertainty flags, and missingness rates. Broad input slices such as insurance status, poverty category, mental health, and chronic-condition count can help explain distribution shifts.</li>
#                 <li><b>Future Monitoring:</b> If the product strategy ever changes to collect actual spend, treat it as a separate opt-in outcome study with explicit consent, data minimization, retention limits, and a privacy review. Because MEPS derives <code>TOTSLF23</code> through detailed expenditure questions, simple user-reported annual spend would be unreliable for many users. Thus, it should be treated as directional unless the collection process can approximate MEPS-quality measurement.</li>
#             </ul>
#     </ul>
# </div>
#
