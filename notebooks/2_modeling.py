...
#         <li><b>Search Spaces:</b> Parameter distributions and iteration counts live in <code><a href="../src/params.py">src/params.py</a></code>.</li>
#         <li><b>Run Tracking:</b> Each script logs the search and best-model run to MLflow and saves the selected model, metrics, parameters, predictions, and full tuning history.</li>
#     </ul>
...
#
# <div style="background-color:#e8f4fd; padding:15px; border:3px solid #d0e7fa; border-radius:6px;">
#     ℹ️ <code>ElasticNet</code> hyperparameter tuning.
...
#         <li><b>Tuning Script:</b> The actual implementation lives in <code><a href="../scripts/tune_elastic_net.py">scripts/tune_elastic_net.py</a></code>.</li>
#     </ul>
# </div>

# %% [markdown]
# <div style="background-color:#fff6e4; padding:15px; border-width:3px; border-color:#f5ecda; border-style:solid; border-radius:6px">
#     📌 Load and review the saved Elastic Net tuning history generated by <code>scripts/tune_elastic_net.py</code>.
# </div>
...
#
# <div style="background-color:#e8f4fd; padding:15px; border:3px solid #d0e7fa; border-radius:6px;">
#     ℹ️ <code>RandomForestRegressor</code> hyperparameter tuning.
#     <ul style="margin-top:8px; margin-bottom:0px">
...
#         <li><b>Search Space:</b> Refer to <code><a href="../src/params.py">src/params.py</a></code> for rationale and parameter distributions.</li>
#         <li><b>Tuning Script:</b> The actual implementation lives in <code><a href="../scripts/tune_random_forest.py">scripts/tune_random_forest.py</a></code>.</li>
#     </ul>
# </div>

# %% [markdown]
# <div style="background-color:#fff6e4; padding:15px; border-width:3px; border-color:#f5ecda; border-style:solid; border-radius:6px">
#     📌 Load and review the saved Random Forest tuning history generated by <code>scripts/tune_random_forest.py</code>.
# </div>
...
#
# <div style="background-color:#e8f4fd; padding:15px; border:3px solid #d0e7fa; border-radius:6px;">
#     ℹ️ <code>XGBRegressor</code> hyperparameter tuning.
#     <ul style="margin-top:8px; margin-bottom:0px">
...
#         <li><b>Search Space:</b> Refer to <code><a href="../src/params.py">src/params.py</a></code> for rationale and parameter distributions.</li>
#         <li><b>Tuning Script:</b> The actual implementation lives in <code><a href="../scripts/tune_xgboost.py">scripts/tune_xgboost.py</a></code>.</li>
#     </ul>
# </div>

# %% [markdown]
# <div style="background-color:#fff6e4; padding:15px; border-width:3px; border-color:#f5ecda; border-style:solid; border-radius:6px">
#     📌 Load and review the saved XGBoost tuning history generated by <code>scripts/tune_xgboost.py</code>.
# </div>
...
#     <br><br>
#     Compares subgroup prediction error across protected, vulnerable, and proxy groups to identify reliability gaps and inform model selection, validation, and monitoring.
# </div> 
...
#     <ul>
#         <li><strong>Sex:</strong> Female/Male disparity is consistent (~1.5×) across all architectures and may reflect differences in utilization and cost variability (e.g., reproductive care). Elastic Net shows the highest disparity (2.1×) due to its exceptionally low male error (\$117).</li>
#         <li><strong>Age:</strong> Prediction error increases with age across all models (4–6× larger error for oldest vs. youngest adults). Architectures converge for the 65+ group (~\$470), consistent with greater clinical complexity and cost variability among older adults.</li>
...
#         <li><strong>Education:</strong> A strong monotonic gradient from No Degree to Doctorate appears across all models, though models diverge at the top: XGBoost (\$837) shows the highest Doctorate MdAE compared to Elastic Net (\$611) and RF (\$577). The overall gradient (No Degree to Doctorate: 8–12× ratio) is consistent with education acting as a proxy for income, insurance, and differences in spending variance.</li>
...
#     </ul>
#     <div style="margin-top:10px;"><strong>Overall Assessment:</strong> Subgroup error patterns are broadly consistent across model architectures, making an architecture-specific fairness issue less likely. Tree-based models are more reliable for several higher-complexity protected and vulnerable groups, while Elastic Net performs best mainly for lower-complexity groups. Together with the heteroscedasticity results, this supported selecting XGBoost for the final quantile regression model, where prediction ranges can communicate uncertainty directly. The audit provides a transparent basis for model selection and identifies higher-error groups for continued validation and monitoring, aligning with NIST AI RMF risk measurement and management practices.</div>
# </div>
# %% [markdown]
...
#     Compare XGBoost with giving everyone the same population quantiles, or the quantiles for their age group (18–34, 35–49, 50–64, and 65+). Both baselines use training data and survey weights to estimate q25, q50, q75, and q90. All three approaches are evaluated on the same test participants. Each approach provides a median estimate (q50), a typical range (q25–q75), and a safety cushion (q90). Population predictions are identical for everyone; age-group predictions are identical within each age band; XGBoost predictions depend on each person’s features.
# </div>
#
# <div style="background-color:#fff6e4; padding:15px; border:3px solid #f5ecda; border-radius:6px;">
...
# %% [markdown]
# <div style="background-color:#fff6e4; padding:15px; border:3px solid #f5ecda; border-radius:6px;">
#     📌 <b>Show error reduction</b><br>
#     Error reduction = 1 − (XGBoost error / baseline error). Positive values favor XGBoost. Brackets show approximate 95% confidence intervals from 1,000 paired bootstrap resamples, using the same sampled participants and survey-weighted metrics for each approach.    
# </div>
...
#     💡 <b>Insights:</b>
#     <ul>
#         <li><b>Plan Around:</b> XGBoost lowers MAE by 9.8% versus the population baseline and 7.6% versus the age-group baseline, with both confidence intervals above zero. Its MdAE is \$240, compared with \$248 and \$305, respectively. The small MdAE improvement over the population baseline remains uncertain.</li>
#         <li><b>Typical Range:</b> XGBoost improves the Winkler score by 11.2% versus the population baseline and 9.0% versus the age-group baseline, with both confidence intervals above zero. This supports the value of personalized ranges when accounting for both their width and penalties for missed outcomes.</li>
#         <li><b>Safety Cushion:</b> XGBoost lowers q90 pinball loss by 15.6% versus the population baseline and 14.3% versus the age-group baseline, with both confidence intervals above zero. All three approaches cover about 90% of outcomes; the model adds value by reducing the penalty for how far predictions miss, with greater weight on underprediction.</li>
#     </ul>
#     <p><b>Conclusion:</b> XGBoost improves predictive performance over population and age-group baselines across MAE, typical-range interval score, and q90 loss. Its added value is less clear for the plan-around estimate, where the population median achieves a similar MdAE. Thus, XGBoost adds value beyond simple baselines, particularly for typical ranges and safety cushions.</p>
# </div>
...
#     <code>src/prediction.py</code> contains the core model inference logic. It takes the preprocessor input features through the fitted preprocessing pipeline and quantile model, converts predictions back to 2023 dollars, postprocesses the four quantiles, and optionally selects q50. It does not cover the complete API/UI prediction request: user-input validation and mapping, inflation adjustment, and response formatting remain outside this module.<br>
#     <code>src/explainability.py</code> contains the SHAP explanation logic for building the explainer and generating prediction explanations. This notebook, the SHAP benchmarking script, the SHAP feature importance script, and the future app import these modules so they all use the same prediction and explanation logic.
#     <br><br>
#     <strong>Feature Importance</strong><br>
#     SHAP feature importance is based on the preprocessor input features produced by several MEPS data preparation steps, but before the preprocessor performs imputation, medical feature engineering, scaling, and one-hot encoding. The callable follows the core model inference flow: <code>27 preprocessor input features → preprocessor → 40 model-ready features → quantile model prediction → inverse target transformation → quantile postprocessing → q50</code>.
#     <br><br>
...
#     <br><br>
#     Benchmarked combinations of background size and permutation rounds, comparing their latency, background validation, and explanation stability. The 225-row background with one permutation round (<code>max_evals=55</code>) had the lowest P95 core SHAP latency among the candidates that passed the quality checks. <a href="../scripts/benchmark_shap.py"><code>scripts/benchmark_shap.py</code></a> contains the benchmark implementation; the SHAP benchmarking section below presents the results.
#     <br><br>
#     <strong>SHAP Metadata</strong><br>
#     The saved <code>app/data/shap_metadata.json</code> records the model and preprocessor artifacts, the SHAP background and explainer settings, and the background validation and final test results that support this configuration. See the <a href="../docs/specs/technical_specifications.md#shap-metadata-contract">SHAP Metadata Artifact Contract</a> in the technical specifications.
#     <br><br>
...
#         <li><strong>Core SHAP explanation latency:</strong> The benchmark measures one shared <code>calculate_shap_explanation(...)</code> call for one validation or test row at a time. This includes repeated masked predictions through the complete q50 callable: preprocessing, quantile prediction, inverse target transformation, quantile postprocessing, and q50 selection. It excludes the other server work, network transfer, and interface rendering.</li>
...
#         <li><strong>Contribution Distributions (Beeswarm Plot):</strong> Shows whether features move estimates up or down and how much their contributions vary across people. For categorical features, it doesn't show which category produced each contribution.</li>
...
#     📌 Create a bar chart of the top 15 features on the test set.
# </div>
...
#                 <li><strong>Socioeconomic:</strong> Lower family income tends to move estimates down, middle income usually moves estimates down but closer to zero, and high income moves estimates up.</li>
...
#         <li><strong>Contribution size varies within categories:</strong> The percentile ranges show that people in the same category can have different contributions, depending on their other feature inputs.</li>
#         <li><strong>Sparse categories:</strong> Interpret results for Separated (1%), Doctorate (3%), GED (4%), and Missing Education (1%) cautiously.</li>
#     </ul>
# </div>
...
#     📌 Zoom in on features with ordered values (numerical and ordinal). Use a dependence plot for numerical features (Age) and an interval plot for ordinal features (Family Income, Family Size) to show how SHAP contributions change across ordered values.
# </div>
...
#     💡 <b>Insights:</b>
#     <ul style="margin-top:10px; margin-bottom:8px">
#         <li><strong>Age contributions change most after the mid-50s:</strong> Age generally moves estimates down at younger ages through middle adulthood. Contributions start rising in the mid-50s and move estimates up around age 60. Contributions also vary much more among older adults.</li>
#         <li><strong>Only High Income moves estimates up:</strong> Poor through Middle Income generally move estimates down, while High Income moves them up.</li>
...
#     💡 <strong>Insights:</strong>
#     <ul>
#         <li>Chronic Conditions Count accounts for the largest share of training split gain (18.6%), followed by Family Income and Insurance (15.0% each). The fitted trees found aggregate medical burden, income, and insurance particularly useful for splits across the four quantiles.</li>
#         <li>This ranking identifies where the trees gained most during training, not the direction or dollar impact of a feature's contribution or its effect on test-set performance.</li>
#     </ul>
# </div>

# %% [markdown]
# <div style="background-color:#f7fff8; padding:15px; border:3px solid #e0f0e0; border-radius:6px;">
#     💡 <strong>SHAP vs. XGBoost:</strong>
#     <ul>
#         <li>The SHAP and consolidated XGBoost rankings share 12 of their top 15 features, including Insurance, Family Income, Age, Education, Family Size, and Usual Source of Care.</li>
#         <li>Chronic Conditions Count and Limitations Count have no SHAP feature importance score because they are derived during preprocessing. SHAP reflects these counts through their source inputs such as individual chronic conditions and limitations.</li>
#         <li>Prioritize SHAP for product-facing interpretation: it measures test-set contributions to the plan-around estimate (q50) in 2023 dollars. Native gain remains a training diagnostic across all four quantiles, so ranks and percentages are not directly comparable.</li>
#     </ul>
# </div>
