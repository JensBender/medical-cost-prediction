...
#         <li><strong>Predicted Cost Tiers:</strong> When stratified by predicted plan-around (q50) or predicted cushion (q90), all groups are well-calibrated, remaining within target bands (typical range: 41.4%–58.2%; safety cushion: 86.5%–91.4%). This confirms that the model's own confidence segmentation is well-calibrated regardless of predicted spend level.</li>
#         <li><strong>Insurance:</strong> Private (49.0% / 90.8%) and public (51.2% / 89.2%) insurance groups are well-calibrated. However, the uninsured group has a typical range coverage of only 34.9% (below the 40% threshold), indicating that predicted ranges are too narrow for the uninsured. This is likely because uninsured costs are more volatile and harder to predict from available features.</li>
...
#         <li><b>Fairness audit groups:</b> sex, age group, race/ethnicity, mental health, income, education, region, and walking limitation.</li>
#         <li><b>Metrics by group:</b> plan-around MdAE (q50), typical range coverage/width (q25-q75), and safety cushion coverage/width (q90 and q50-q90).</li>
#         <li><b>Coverage review triggers:</b> Release gates apply to overall test coverage (45-55% for typical range, 85-95% for safety cushion). Subgroup test coverage thresholds are more lenient diagnostic triggers, not additional pass/fail gates. For subgroups with sufficient sample size (n ≥ 30), typical range coverage should generally stay within 40-60%, and safety cushion coverage should generally stay within 80-97%. Small subgroups (n &lt; 30) are labeled review-only because their estimates are less stable.</li>
...
#         <li><b>Interpretation:</b> Use subgroup flags to qualify reporting, document caveats, and prioritize post-launch monitoring. Because this is the locked test set, flagged subgroups should not drive another tuning loop; follow-up changes should be evaluated in a new validation cycle.</li>
#     </ul>
# </div>
...
#     <ul>
#         <li><strong>Actual Cost Tiers:</strong> The biggest caveat is predictability of high actual costs. Actual High spenders have 12.4% typical-range coverage and 59.0% safety-cushion coverage; actual Very High spenders have 0.0% and 6.7%, respectively. Zero- and low-cost actual groups are heavily overprotected by the safety cushion (100.0% and 99.9%). This is not a demographic fairness pattern; it is the expected ex-post tail-cost limitation of zero-inflated, heavy-tailed medical costs.</li>
#         <li><strong>Predicted Cost Tiers:</strong> The deployable risk tiers are much better behaved because they are known at prediction time. Predicted plan-around cost tiers keep typical-range coverage inside the diagnostic band (42.6%-54.6%), and predicted safety-cushion tiers keep q90 coverage inside the diagnostic band (85.6%-93.4%). Widths also increase monotonically with predicted risk, from a \$1,125 safety-cushion width in the predicted q90 Low tier to \$5,582 in the predicted q90 Very High tier, which supports using predicted-risk bands for user-facing uncertainty language.</li>
#         <li><strong>Clinical and Coverage Reliability:</strong> Physical health, chronic conditions, private insurance, and public insurance groups remain within diagnostic coverage bands. Uninsured users are the main coverage watchlist group: typical-range coverage is low at 34.7%, while safety-cushion coverage is conservative at 96.3%. The safety cushion is not under-warning, but the typical range may be too narrow or shifted for uninsured users and should be monitored after launch.</li>
#         <li><strong>Demographic Fairness:</strong> Sex, age, race/ethnicity, region, and walking limitation groups do not show systematic undercoverage. Subgroups to watch are poor mental health (30.1% typical-range coverage), low income (39.2%), doctorate degree (34.7%), and near poor (97.7% safety-cushion overcoverage). These should qualify reporting and monitoring, not trigger test-set-driven model retuning.</li>
#         <li><strong>Usefulness Caveat:</strong> Several low-cost groups have wide intervals despite acceptable coverage, including good physical health, good mental health, Asian respondents, and West region. This is a practical-budgeting caveat rather than a safety failure, because the model often protects users in these subgroups from underestimation, but some low-cost users may receive ranges that are less specific than desired.</li>
#     </ul>
# </div>
