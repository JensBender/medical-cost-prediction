...
#     <ol>
#         <li><strong>Feature Importance Analysis for Model Audit:</strong> Rank features by mean absolute SHAP value (<code>mean(|SHAP|)</code>) across many predictions to identify which features most affect the post-processed q50 estimate. Compare with XGBoost native importance to audit whether the model relies on plausible cost drivers. Use <code>total_gain</code> (total training-loss reduction from all splits on a feature) and <code>total_cover</code> (total training samples flowing through splits on a feature) for native importance, since split count (<code>weight</code>) only measures how often a feature was used. These methods answer related but different questions: SHAP measures each feature's dollar impact on individual predictions, while native importance measures each feature's role during training.</li>
