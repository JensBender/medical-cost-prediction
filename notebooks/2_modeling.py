...
#         <li><b>Interpretation:</b> Use diagnostic flags to qualify reporting, document caveats, and prioritize privacy-preserving post-launch drift monitoring or future offline validation. Because this is the locked test set, flagged subgroups should not drive another tuning loop; follow-up changes should be evaluated in a new validation cycle.</li>
#     </ul>
# </div>
...
#         <li><b>Calibration Decision:</b> Do not add conformalized quantile regression. Test-set calibration passes the predefined gates, so any calibration changes should be evaluated only in a new validation cycle or a future MEPS-based offline validation review.</li>
...
#         <li><b>Model Monitoring:</b> For the privacy-first MVP, do not claim post-launch calibration from app users. True calibration requires actual annual out-of-pocket spending, but collecting linked follow-up outcomes would be unreliable for many users and would weaken the anonymous, zero-retention product requirement.</li>
#         <li><b>MVP Monitoring Scope:</b> Track aggregate app health, completion rate, input drift, and prediction drift without storing user-level input rows or prediction records. Useful aggregate slices include predicted q90 tier, insurance status, poverty category, mental health, chronic-condition count, zero/low predicted-cost profiles, interval width, and missingness rates.</li>
#         <li><b>Future Outcome Monitoring:</b> If the product strategy ever changes to collect actual spend, treat it as a separate opt-in outcome study with explicit consent, data minimization, retention limits, and a privacy review. Because MEPS derives <code>TOTSLF23</code> through detailed expenditure questions, simple user-reported annual spend should be treated as directional unless the collection process can approximate MEPS-quality measurement. Otherwise, use future MEPS releases for offline recalibration checks.</li>
#     </ul>
# </div>
#
