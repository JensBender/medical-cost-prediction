...
#     <ol>
#         <li><strong>Feature Importance Analysis for Model Audit:</strong> Aggregate SHAP values across many samples to estimate which features most move the final post-processed q50 dollar estimate across users, using mean absolute SHAP values (<code>mean(abs(SHAP value))</code>). Compare this with XGBoost native importance to audit whether the model relies on plausible cost drivers. Native XGBoost importance answers which features the inner trees relied on most while fitting, such as split frequency, gain, or cover. Use <code>total_gain</code> for XGBoost native importance because it measures total training-loss reduction from all splits using a feature, whereas split count (<code>weight</code>) only measures how often a feature was used in splits. These methods answer related but different questions.</li>
...
#         <li><strong>Correlated Features:</strong> EDA shows the most notable feature correlations for ADL/IADL help (Spearman's ρ=0.60) and arthritis/joint pain (0.56). SHAP credit can therefore split or shift within these groups. For example, if arthritis and joint pain both signal similar health burden, one prediction may assign more impact to arthritis and another to joint pain even when the broader pattern is similar. Interpret linked features as groups, and avoid treating small ranking differences as precise.</li>
