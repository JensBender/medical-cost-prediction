...
#     <ul>
#         <li><strong>Actual cost range:</strong> MdAE increases exponentially across actual cost ranges for all three models. At the extremes, the models converge: Zero Costs MdAE ranges from \$29 (XGBoost) to \$90 (Elastic Net), and Very High Spend (Top 5%) from \$9,469 (XGBoost) to \$9,919 (Elastic Net). This tight convergence at the extremes confirms that extreme-cost prediction difficulty is a <em>data-driven</em> limitation, not model-specific. Elastic Net shows notably higher errors for Zero Costs (\$90 vs. \$29–\$31 for tree-based models), because its linear structure cannot model the sharp boundary at zero spending.</li>
#         <li><strong>Predicted cost range:</strong> All three models compress their output range, but the degree differs. For predicted "Very High Spend", Elastic Net retains the most signal (\$1,189 MdAE) while Random Forest (\$699) and XGBoost (\$926) exhibit more aggressive attenuation. This reveals that tree-based models' ensemble averaging attenuates tail predictions more aggressively than regularized linear models. The compression is a conservative bias acceptable for an advisory tool but important to disclose.</li>
#         <li><strong>Health:</strong> Physical health errors increase from Excellent to Poor across all models. The model families diverge at the Poor stratum: Elastic Net (\$799) produces nearly 2× the error of XGBoost (\$413) and Random Forest (\$437). Tree-based models better capture the non-linear health-cost relationship for the most complex patients, while Elastic Net's linear assumptions break down in this tail.</li>
#         <li><strong>Insurance:</strong> All models agree on the rank order: Private > Public > Uninsured. However, the Uninsured gap is striking — Elastic Net (\$95) shows 3–4× the MdAE of tree models (\$31 RF, \$26 XGBoost). Tree models learn the sharp spending constraint for uninsured individuals, while Elastic Net's smooth predictions overshoot this near-zero cluster. Private insurance errors are more consistent across architectures (\$262–\$322).</li>
#         <li><strong>Chronic conditions:</strong> All models show rising MdAE from 0 conditions (\$92–\$108) through 3 conditions (\$453–\$533). At 4+ conditions, tree models plateau or decrease (RF \$517, XGBoost \$489) while Elastic Net jumps to \$799, a 1.6× gap over tree models. This confirms the saturation effect for tree-based architectures, where extreme chronic burden becomes a more stable (and thus more predictable) cost driver. Elastic Net lacks the capacity to model this non-linear plateau.</li>
#     </ul>
#     <div style="margin-top:10px;"><strong>Fairness Audit</strong></div>
#     <ul>
#         <li><strong>Sex:</strong> The Female/Male disparity ratio is consistent across all models: 1.5× (XGBoost \$308/\$207, RF \$263/\$201, Elastic Net \$247/\$117). The fact that three different architectures produce a similar gap suggests this reflects genuine utilization variance (reproductive care, preventive screenings), not algorithmic discrimination. Notably, Elastic Net shows the lowest absolute female error (\$247) but the highest disparity ratio (2.1×) due to its particularly low male error (\$117).</li>
#         <li><strong>Age:</strong> Prediction error increases with age across all models, from 18-34 (\$88–\$116) to 65+ (\$453–\$486), a 4–6× ratio. Models converge closely for older groups (65+: \$453–\$486) but diverge for younger groups where Elastic Net's linear assumptions yield lower errors (\$88 vs. \$116 XGBoost for 18-34). Increasing prediction error with age across all architectures suggests clinical complexity, not age-based bias.</li>
#         <li><strong>Race/Ethnicity:</strong> All models agree: White (\$317–\$336) shows the highest MdAE, while Hispanic (\$95–\$103) and Black (\$101–\$139) show lower errors. The rank order is preserved across architectures, with tight inter-model agreement for Hispanic (\$95–\$103, 8% spread) and broader but directionally consistent agreement for Black (\$101–\$139). This cross-model convergence rules out architecture-specific bias. The lower MdAE for minority groups reflects more concentrated spending distributions, not disparate impact.</li>
#         <li><strong>Income gradient:</strong> MdAE rises from Low to High Income across all models: Elastic Net \$88→\$350 (4.0×), RF \$76→\$320 (4.2×), XGBoost \$53→\$354 (6.7×). XGBoost shows the steepest gradient, driven by its superior precision for low-income groups (\$53 vs. \$76–\$88). The pattern mirrors insurance and access dynamics: higher-income individuals use more diverse and expensive services. Low-income groups have more constrained, predictable spending.</li>
#         <li><strong>Education:</strong> A strong monotonic gradient from No Degree to Doctorate across all models, though models diverge at the top: XGBoost (\$837) shows the highest Doctorate MdAE compared to Elastic Net (\$611) and RF (\$577). The overall gradient (No Degree to Doctorate: 8–12× ratio) confirms education acts as a proxy for income and insurance quality. This is a Legitimate Business Necessity disparity, not bias.</li>
#         <li><strong>Mental health:</strong> Errors rise from Excellent to Poor across all models: Elastic Net \$115→\$644, RF \$223→\$510, XGBoost \$206→\$594. An interesting divergence appears at Excellent and Fair: Elastic Net is the most precise for healthy individuals (\$115 vs. \$207–\$223) but the least precise for Poor (\$644 vs. \$510–\$594). Mental health severity correlates with complex comorbidities where non-linear models have an advantage.</li>
#         <li><strong>Walking limitations:</strong> The limitation/no-limitation ratio is consistent: 2.5× (XGBoost \$533/\$216), 2.6× (RF \$536/\$210), and 4.2× (Elastic Net \$612/\$146). Elastic Net's higher ratio is driven by both a lower no-limitation error and a higher limitation error, reflecting that linear models struggle more with the medical complexity associated with disability.</li>
#         <li><strong>Region:</strong> Modest variation across all models, with the South consistently lowest (\$142–\$216) and Northeast/Midwest highest (\$241–\$322). The inter-model spread within each region is small (e.g., South: \$142–\$216, Midwest: \$241–\$322), confirming the smallest disparity of any audited dimension. No evidence of geographic bias requiring intervention.</li>
#     </ul>
#     <div style="margin-top:10px;"><strong>Overall Assessment:</strong> All three model architectures produce the same directional disparities across protected groups, confirming these reflect data-generating characteristics (utilization patterns, access constraints) rather than architecture-specific algorithmic bias. Tree-based models (XGBoost, Random Forest) consistently outperform Elastic Net for high-complexity segments (poor health, many chronic conditions, walking limitations), while Elastic Net shows competitive or superior errors for low-complexity, low-cost segments. No evidence of discriminatory disparate impact against protected groups was found across any model. All three models are suitable for deployment as low-risk advisory tools under NIST/FTC transparency guidelines.</div>
# </div>
# %% [markdown]
