...
#
# <div style="background-color:#f7fff8; padding:15px; border:3px solid #e0f0e0; border-radius:6px;">
#     💡 <b>EDA-Driven Decisions for Modeling</b> 
#     <ul style="margin-top:10px; margin-bottom:0px">
#         <li style="margin-bottom:10px;"><b>1. Model Evaluation & Data Split</b>
...
#                 <li><b>Splitting:</b> Use a <b>High-Resolution Stratified Split</b> ($0, 50, 80, 95, 99, 99.9\%$) to guarantee the validation and test sets accurately mirror the full cost distribution, including the zero-hurdle and high-cost tail.</li>
#             </ul>
#         </li>
#         <li style="margin-bottom:10px;"><b>2. Feature Engineering</b>
#             <ul style="margin-top:5px;">
#                 <li><b>Limitations and Condition Counts:</b> Chronic conditions and functional limitations are highly inter-correlated (HBP↔Cholesterol 0.45, Arthritis↔Joint Pain 0.56, ADL↔IADL 0.60). This shared signal justifies engineering <code>CHRONIC_COUNT</code> and <code>LIMITATION_COUNT</code> as aggregate burden indices, reducing redundancy for regression-based models susceptible to multicollinearity while preserving cumulative signal.</li>
#                 <li><b>Life Transitions and Sparse Categories:</b> The "Married in Round" category (\$788 median vs. \$410 for stable "Married") confirms that transition categories carry a meaningful cost premium. This validates the <code>RECENT_LIFE_TRANSITION</code> flag and the decision to collapse sparse transition categories into their stable counterparts (<code>MARRY31X_GRP</code>, <code>EMPST31_GRP</code>).</li>
#                 <li><b>No Further Engineering Needed:</b> Non-linear patterns (Age scatter, Poverty staircase) do not require manual binning or interaction terms—tree-based models capture these natively, and polynomial features handle them for linear models. Weak but present signals (Smoker ρ=−0.04, Mental Health ρ=0.02) are retained rather than dropped, since tree-based models can still leverage them in interaction splits.</li>
#             </ul>
#         </li>
#         <li style="margin-bottom:10px;"><b>3. Model Pipeline</b>
#             <ul style="margin-top:5px;">
#                 <li><b>Lean Pipeline:</b> Tree-based models (XGBoost, Random Forest, Decision Tree), MLP, SVM, and KNN use the core pipeline, relying on their native ability to capture non-linear relationships and interactions.</li>
#                 <li><b>Polynomial Pipeline:</b> Regression-based models (Linear, Elastic Net) require explicit polynomial features to simulate the non-linear patterns and interaction effects that tree models learn natively.</li>
#             </ul>
#         </li>
#     </ul>
# </div> 
